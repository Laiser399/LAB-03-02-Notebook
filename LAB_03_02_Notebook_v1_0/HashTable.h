...
{
private:
	std::vector<std::vector<std::pair<key_t, value_t> > > table; // хэш иаблица
	int currDepth, maxDepth, hashMulti; // глубина, максимально допустимая глубина, хэш множитель
	bool canRebuild;

	int hashFunc(key_t &key); // хэш функция
	void rebuild();
public:
	HashTable();
...
HashTable<key_t, value_t>::HashTable(HashTable<key_t, value_t> &another) {
	...
	{
		// цикл по массиву в ячейке таблицы
...
void HashTable<key_t, value_t>::clear() {
    for (auto it = table.begin(), it_l = table.end(); it != it_l; ++it) {
        it->resize(0);
    }
    currDepth = 0;
}
